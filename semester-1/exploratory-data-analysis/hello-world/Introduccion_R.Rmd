...
```{R matrizExample}
x <- matrix(1:20, nrow=5, ncol=4, TRUE)
x
print("")
class(x)
```
...
```{R ejercicioFactor1}
# 1 linea de c?digo
l <- c('Poor', 'Improved', 'Excellent')
PatientStatus <- factor(l, levels = l)
# -------------------
PatientStatus
```
...
```{R ejercicioFactor3}
# 2 lineas de c?digo
x <- factor(x <- c(11, 22, 47, 47, 11, 47, 11), levels = c(11, 22, 47), ordered = TRUE)
...
df <- read.csv("parks.csv", stringsAsFactors = FALSE)
# --------------------------- #
```
...
library("xlsx")
workbook <- "" # direcci?n del archivo
